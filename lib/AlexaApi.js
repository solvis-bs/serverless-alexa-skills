...
class AlexaApi {
  ...
  getSkills(vendorId) {
    return BbPromise.bind(this)
      ...
      .filter((skill) => skill.stage === 'development');
  }
  getModels(vendorId) {
    return BbPromise.bind(this)
      .then(function () {
  ...
  checkModelUpdateStatus(url, locale) {
    return this.getUpdateStatus(url).then((resp) => {
  ...
  createSkill(vendorId, locales, type) {
  ...
  createSkillWithManifest(vendorId, manifest) {
    return this.post('/skills', {
      vendorId,
      manifest: manifest,
    })
  ...
  getAccountLinkings(vendorId) {
    return BbPromise.bind(this)
      .then(function () {
        return BbPromise.resolve(this.getSkills(vendorId));
      })
      .mapSeries(function (skill) {
        return this.get(`/skills/${skill.skillId}/stages/development/accountLinkingClient`)
          .then((body) =>
            BbPromise.resolve({
              id: skill.skillId,
              accountLinking: JSON.parse(body),
              skill,
            })
          )
          .catch((err) => {
  ...
  updateAccountLinking(skillId, accountLinkingRequest) {
    return this.put(`/skills/${skillId}/stages/development/accountLinkingClient`, { accountLinkingRequest }).then(() =>
      BbPromise.resolve({ id: skillId })
    );
  }
}
