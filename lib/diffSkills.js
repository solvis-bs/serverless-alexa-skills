    ...
    return BbPromise.bind(this)
      ...
      .map(function (remote) {
        ...
        return BbPromise.resolve({
          ...
          diff: local ? diff(remote.manifest, local.manifest) : null
        });
      })
      .then(function (ret) {
        const localSkills = this.serverless.service.custom.alexa.skills;
        ...
        localSkills.forEach(function (local) {
          if (!ret.find(entry => entry.local === local)) {
            const data = getLocaleData(local);
            if (!local.id && !data) {
              throw new Error('Cannot create skill without at least one locale name in the manifest (publishingInformation.locales.xxx.name)');
            }
            ret.push({
              skillId: null,
              local: local,
